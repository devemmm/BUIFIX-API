...
const getWageEmployees = async (req) => {
    try {
        const { user, employee } = req;

        const wageEmployee = await Wage_Employee.find({ owner: employee?.owner ? employee.owner : user._id, active: true });
        return wageEmployee;
    } catch (error) {
        throw new Error(error.message);
    }
...
const getActivities = async (req) => {
    try {
        ...
        const owner = await Owner.findOne({ uid: employee?.owner ? employee.owner : user._id });
...
const updateActivityProgress = async (req, id, expenses) => {
    try {
